#include "_cgen.h"

// ── Fn body / decl ────────────────────────────────────────

/** Return true if the top-level block contains any TT_DEFER nodes. */
static bool block_has_defers(const TTNode *body) {
    if (body == NULL || body->kind != TT_BLOCK) {
        return false;
    }
    for (int32_t i = 0; i < BUF_LEN(body->block.stmts); i++) {
        if (body->block.stmts[i]->kind == TT_DEFER) {
            return true;
        }
    }
    return false;
}

/** Emit defer bodies in LIFO order and the cleanup return. */
static void emit_deferred_cleanup(CGen *cgen, const Type *return_type) {
    cgen_emit_line(cgen, "_rsg_cleanup:;");
    for (int32_t i = BUF_LEN(cgen->defer_bodies) - 1; i >= 0; i--) {
        const TTNode *defer_node = cgen->defer_bodies[i];
        if (defer_node->defer_stmt.body != NULL) {
            cgen_emit_stmt(cgen, defer_node->defer_stmt.body);
        }
    }
    if (return_type != NULL && return_type->kind != TYPE_UNIT) {
        cgen_emit_line(cgen, "return _rsg_result;");
    }
}

/**
 * Emit the body of a fn: block stmts, trailing result
 * expr, and an implicit `return 0;` for main.
 */
static void emit_fn_body(CGen *cgen, const TTNode *fn_node) {
    const TTNode *body = fn_node->fn_decl.body;
    const Type *return_type = fn_node->fn_decl.return_type;
    bool is_unit = return_type == NULL || return_type->kind == TYPE_UNIT;
    bool is_main = strcmp(fn_node->fn_decl.name, "main") == 0;
    bool has_defers = block_has_defers(body);

    // Save and set defer state
    const TTNode **saved_defers = cgen->defer_bodies;
    bool saved_in_deferred = cgen->in_deferred_fn;
    cgen->defer_bodies = NULL;
    cgen->in_deferred_fn = has_defers;

    if (has_defers && !is_unit && !is_main) {
        cgen_emit_line(cgen, "%s _rsg_result;", cgen_c_type_for(cgen, return_type));
    }

    if (body != NULL && body->kind == TT_BLOCK) {
        // Block body with optional trailing result
        cgen_emit_block_stmts(cgen, body);

        if (body->block.result != NULL) {
            const TTNode *result_node = body->block.result;
            // Statement-like results need special handling
            if (result_node->kind == TT_ASSIGN) {
                // Emit as stmt (side-effect only in fn body)
                cgen_emit_stmt(cgen, result_node);
            } else if (!is_unit && !is_main) {
                const char *result = cgen_emit_expr(cgen, result_node);
                if (has_defers) {
                    cgen_emit_line(cgen, "_rsg_result = %s;", result);
                    cgen_emit_line(cgen, "goto _rsg_cleanup;");
                } else {
                    cgen_emit_line(cgen, "return %s;", result);
                }
            } else {
                // Unit/main: evaluate for side effects
                if (result_node->kind == TT_CALL) {
                    const char *result = cgen_emit_expr(cgen, result_node);
                    cgen_emit_line(cgen, "%s;", result);
                } else if (result_node->kind == TT_IF) {
                    cgen_emit_if(cgen, result_node, NULL, false);
                } else {
                    const char *result = cgen_emit_expr(cgen, result_node);
                    cgen_emit_line(cgen, "(void)%s;", result);
                }
            }
        }
    } else if (body != NULL) {
        // Expression body (fn foo() = expr)
        const char *result = cgen_emit_expr(cgen, body);
        if (!is_unit && !is_main) {
            if (has_defers) {
                cgen_emit_line(cgen, "_rsg_result = %s;", result);
                cgen_emit_line(cgen, "goto _rsg_cleanup;");
            } else {
                cgen_emit_line(cgen, "return %s;", result);
            }
        } else {
            cgen_emit_line(cgen, "(void)%s;", result);
        }
    }

    if (has_defers) {
        emit_deferred_cleanup(cgen, is_unit ? NULL : return_type);
    }

    // Restore defer state
    cgen->defer_bodies = saved_defers;
    cgen->in_deferred_fn = saved_in_deferred;
}

/** Emit the fn signature: return-type name(params). */
static void emit_fn_signature(CGen *cgen, const TTNode *node) {
    const char *prefix = node->fn_decl.is_pub ? "" : "static ";
    const char *return_type = cgen_c_type_for(cgen, node->fn_decl.return_type);
    const char *fn_name = node->fn_decl.sym->mangled_name;

    cgen_emit_indent(cgen);
    fprintf(cgen->output, "%s%s %s(", prefix, return_type, fn_name);

    int32_t param_count = BUF_LEN(node->fn_decl.params);
    if (param_count == 0) {
        fprintf(cgen->output, "void");
    } else {
        for (int32_t i = 0; i < param_count; i++) {
            const TTNode *param = node->fn_decl.params[i];
            const Type *param_type = param->param.param_type;
            if (param_type == NULL) {
                param_type = &TYPE_I32_INST;
            }
            if (i > 0) {
                fprintf(cgen->output, ", ");
            }
            if (param->param.is_recv) {
                if (!param->param.is_ptr_recv) {
                    // Value recv: pass by value (copy)
                    fprintf(cgen->output, "%s %s", cgen_c_type_for(cgen, param_type),
                            param->param.sym->mangled_name);
                } else if (param->param.is_mut_recv) {
                    fprintf(cgen->output, "%s *%s", cgen_c_type_for(cgen, param_type),
                            param->param.sym->mangled_name);
                } else {
                    fprintf(cgen->output, "const %s *%s", cgen_c_type_for(cgen, param_type),
                            param->param.sym->mangled_name);
                }
            } else {
                fprintf(cgen->output, "%s %s", cgen_c_type_for(cgen, param_type),
                        param->param.sym->mangled_name);
            }
        }
    }
}

static void emit_fn_decl(CGen *cgen, const TTNode *node, bool forward_only) {
    emit_fn_signature(cgen, node);

    if (forward_only) {
        fprintf(cgen->output, ");\n");
        return;
    }

    fprintf(cgen->output, ") {\n");
    cgen->indent++;
    emit_fn_body(cgen, node);
    cgen->indent--;
    cgen_emit_line(cgen, "}");
    fprintf(cgen->output, "\n");
}

// ── Preamble + file emission ───────────────────────────────────────────

/** Emit the preamble: generated-file warning and required C headers. */
static void emit_preamble(CGen *cgen) {
    _cgen_emit(cgen, "// Generated by resurg compiler - do not edit.\n");
    _cgen_emit(cgen, "#include <stdint.h>\n");
    _cgen_emit(cgen, "#include <stdbool.h>\n");
    _cgen_emit(cgen, "#include \"rsg_runtime.h\"\n\n");
}

/** Return true if @p node is a top-level stmt. */
static bool is_top_level_stmt(const TTNode *node) {
    return node->kind != TT_MODULE && node->kind != TT_FN_DECL && node->kind != TT_TYPE_ALIAS &&
           node->kind != TT_STRUCT_DECL && node->kind != TT_ENUM_DECL;
}

/**
 * Emit the auto-generated `main()` entry point that calls
 * `_rsg_top_level()`, the user's `main()`, and all zero-param
 * `test_*` fns.
 */
static void emit_entry_point(CGen *cgen, const TTNode *file, bool has_top_stmts) {
    _cgen_emit(cgen, "int main(void) {\n");
    cgen->indent++;

    // Initialise the tracing GC with the stack bottom anchor.
    cgen_emit_line(cgen, "int _rsg_gc_anchor;");
    cgen_emit_line(cgen, "rsg_gc_init(&_rsg_gc_anchor);");

    if (has_top_stmts) {
        cgen_emit_line(cgen, "_rsg_top_level();");
    }

    for (int32_t i = 0; i < BUF_LEN(file->file.decls); i++) {
        const TTNode *decl = file->file.decls[i];
        if (decl->kind != TT_FN_DECL) {
            continue;
        }
        const char *name = decl->fn_decl.name;
        bool is_main = strcmp(name, "main") == 0;
        bool is_test = strncmp(name, "test_", 5) == 0;
        bool has_params = BUF_LEN(decl->fn_decl.params) != 0;
        if ((is_main || is_test) && !has_params) {
            cgen_emit_line(cgen, "%s();", decl->fn_decl.sym->mangled_name);
        }
    }

    cgen_emit_line(cgen, "return 0;");
    cgen->indent--;
    _cgen_emit(cgen, "}\n");
}

/** Emit module comment and register compound types. */
static void emit_module_and_types(CGen *cgen, const TTNode *file) {
    for (int32_t i = 0; i < BUF_LEN(file->file.decls); i++) {
        const TTNode *decl = file->file.decls[i];
        if (decl->kind == TT_MODULE) {
            cgen->module = decl->module.name;
            _cgen_emit(cgen, "// module %s\n\n", decl->module.name);
        }
    }

    cgen_clear_compound_types(cgen);
    for (int32_t i = 0; i < BUF_LEN(file->file.compound_types); i++) {
        BUF_PUSH(cgen->compound_types, file->file.compound_types[i]);
    }
    cgen_emit_compound_typedefs(cgen);
}

/** Emit forward decls, then full defs, for all fns. */
static void emit_fns(CGen *cgen, const TTNode *file) {
    for (int32_t i = 0; i < BUF_LEN(file->file.decls); i++) {
        const TTNode *decl = file->file.decls[i];
        if (decl->kind == TT_FN_DECL) {
            emit_fn_decl(cgen, decl, true);
        }
    }
    _cgen_emit(cgen, "\n");

    for (int32_t i = 0; i < BUF_LEN(file->file.decls); i++) {
        const TTNode *decl = file->file.decls[i];
        if (decl->kind == TT_FN_DECL) {
            emit_fn_decl(cgen, decl, false);
        }
    }
}

/** Emit `_rsg_top_level()` wrapper if any top-level stmts exist. */
static bool emit_top_level_wrapper(CGen *cgen, const TTNode *file) {
    bool has_top_stmts = false;
    for (int32_t i = 0; i < BUF_LEN(file->file.decls); i++) {
        if (is_top_level_stmt(file->file.decls[i])) {
            has_top_stmts = true;
            break;
        }
    }
    if (!has_top_stmts) {
        return false;
    }

    _cgen_emit(cgen, "static void _rsg_top_level(void) {\n");
    cgen->indent++;
    for (int32_t i = 0; i < BUF_LEN(file->file.decls); i++) {
        if (is_top_level_stmt(file->file.decls[i])) {
            cgen_emit_stmt(cgen, file->file.decls[i]);
        }
    }
    cgen->indent--;
    _cgen_emit(cgen, "}\n\n");
    return true;
}

/**
 * Emit the full C translation unit: preamble, module comment, compound
 * types, forward decls, fn defs, and (if present)
 * a `_rsg_top_level()` wrapper for top-level stmts.
 */
static void emit_file(CGen *cgen, const TTNode *file) {
    emit_preamble(cgen);
    emit_module_and_types(cgen, file);
    emit_fns(cgen, file);
    bool has_top_stmts = emit_top_level_wrapper(cgen, file);
    emit_entry_point(cgen, file, has_top_stmts);
}

// ── Public API ─────────────────────────────────────────────────────────

CGen *cgen_create(FILE *output, Arena *arena) {
    CGen *cgen = rsg_malloc(sizeof(*cgen));
    cgen->output = output;
    cgen->arena = arena;
    cgen->indent = 0;
    cgen->module = NULL;
    cgen->temp_counter = 0;
    cgen->str_builder_counter = 0;
    cgen->compound_types = NULL;
    cgen->defer_bodies = NULL;
    cgen->in_deferred_fn = false;
    return cgen;
}

void cgen_destroy(CGen *cgen) {
    if (cgen != NULL) {
        cgen_clear_compound_types(cgen);
        free(cgen);
    }
}

void cgen_emit(CGen *cgen, const TTNode *file) {
    emit_file(cgen, file);
}
