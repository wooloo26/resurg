...
static const char *join_exprs(CGen *cgen, TTNode **nodes, int32_t count) {
    const char *result = "";
    for (int32_t i = 0; i < count; i++) {
        const char *elem = cgen_emit_expr(cgen, nodes[i]);
        result = (i == 0) ? elem : arena_sprintf(cgen->arena, "%s, %s", result, elem);
    }
    return result;
}

static const char *emit_bool_lit(const TTNode *node) {
    return node->bool_lit.value ? "true" : "false";
}

static const char *emit_int_lit(CGen *cgen, const TTNode *node) {
...
static const char *emit_float_lit(CGen *cgen, const TTNode *node) {
    if (node->float_lit.float_kind == TYPE_F32) {
        return cgen_fmt_float32(cgen, node->float_lit.value);
    }
    return cgen_fmt_float64(cgen, node->float_lit.value);
}

static const char *emit_str_lit(CGen *cgen, const TTNode *node) {
    const char *escaped = cgen_c_str_escape(cgen, node->str_lit.value);
    return arena_sprintf(cgen->arena, "rsg_str_lit(\"%s\")", escaped);
}

static const char *emit_unary_expr(CGen *cgen, const TTNode *node) {
    const char *operand = cgen_emit_expr(cgen, node->unary.operand);
...
static const char *emit_binary_expr(CGen *cgen, const TTNode *node) {
    const char *left = cgen_emit_expr(cgen, node->binary.left);
    const char *right = cgen_emit_expr(cgen, node->binary.right);
...
static bool is_rsg_assert_call(const TTNode *node) {
    if (node->call.callee->kind != TT_VAR_REF) {
        return false;
    }
    return strcmp(tt_sym_name(node->call.callee->var_ref.sym), "rsg_assert") == 0;
}
 ...
 */
static const char *emit_rsg_assert_call(CGen *cgen, const TTNode *node) {
    const char *cond = cgen_emit_expr(cgen, node->call.args[0]);

    const TTNode *msg_node = node->call.args[1];
    const char *msg;
    if (msg_node->kind == TT_UNIT_LIT) {
        msg = "NULL";
    } else if (msg_node->kind == TT_STR_LIT) {
        msg =
            arena_sprintf(cgen->arena, "\"%s\"", cgen_c_str_escape(cgen, msg_node->str_lit.value));
    } else {
        const char *msg_expr = cgen_emit_expr(cgen, msg_node);
        msg = arena_sprintf(cgen->arena, "%s.data", msg_expr);
    }

    const TTNode *file_node = node->call.args[2];
    const char *file =
        arena_sprintf(cgen->arena, "\"%s\"", cgen_c_str_escape(cgen, file_node->str_lit.value));

    const char *line = cgen_emit_expr(cgen, node->call.args[3]);

    return arena_sprintf(cgen->arena, "rsg_assert(%s, %s, %s, %s)", cond, msg, file, line);
}

static const char *emit_call_expr(CGen *cgen, const TTNode *node) {
    ...
    if (node->call.callee->kind == TT_VAR_REF) {
        ...
        callee = cgen_emit_expr(cgen, node->call.callee);
    }
...
static bool is_pure_expr(const TTNode *node) {
    if (node == NULL) {
        return false;
    }
    switch (node->kind) {
...
static bool is_ternary_candidate(const TTNode *node) {
    if (node->kind != TT_IF || node->if_expr.else_body == NULL) {
        return false;
    }
    const TTNode *then_result = simple_branch_result(node->if_expr.then_body);
    if (then_result == NULL || !is_pure_expr(then_result)) {
        return false;
    }
    const TTNode *else_body = node->if_expr.else_body;
...
static const char *emit_if_expr(CGen *cgen, const TTNode *node) {
    // Emit as C ternary when both branches are pure single-expr results
    if (is_ternary_candidate(node)) {
        const char *cond = cgen_emit_expr(cgen, node->if_expr.cond);
        const TTNode *then_result = simple_branch_result(node->if_expr.then_body);
        const TTNode *else_result = simple_branch_result(node->if_expr.else_body);
        const char *then_value = cgen_emit_expr(cgen, then_result);
        const char *else_value = cgen_emit_expr(cgen, else_result);
    ...
    const char *temp = cgen_next_temp(cgen);
    cgen_emit_line(cgen, "%s %s;", cgen_c_type_for(cgen, type), temp);
    cgen_emit_if(cgen, node, temp, false);
    return temp;
}

static const char *emit_block_expr(CGen *cgen, const TTNode *node) {
    cgen_emit_block_stmts(cgen, node);
    if (node->block.result != NULL) {
        return cgen_emit_expr(cgen, node->block.result);
    }
    return "(void)0";
}

static const char *emit_array_lit_expr(CGen *cgen, const TTNode *node) {
    const char *tname = cgen_c_type_for(cgen, node->type);
...
static const char *emit_slice_lit_expr(CGen *cgen, const TTNode *node) {
    int32_t count = BUF_LEN(node->slice_lit.elems);
    const Type *elem_type = node->type->slice.elem;
    const char *elem_c = cgen_c_type_for(cgen, elem_type);
    if (count == 0) {
...
static const char *emit_slice_expr_expr(CGen *cgen, const TTNode *node) {
    const char *object = cgen_emit_expr(cgen, node->slice_expr.object);
    ...
    if (node->slice_expr.start != NULL) {
        start_str = cgen_emit_expr(cgen, node->slice_expr.start);
    }
    const char *end_str;
    if (node->slice_expr.end != NULL) {
        end_str = cgen_emit_expr(cgen, node->slice_expr.end);
    } else {
...
static const char *emit_tuple_lit_expr(CGen *cgen, const TTNode *node) {
    const char *tname = cgen_c_type_for(cgen, node->type);
    ...
    for (int32_t i = 0; i < BUF_LEN(node->tuple_lit.elems); i++) {
        ...
        const char *elem = cgen_emit_expr(cgen, node->tuple_lit.elems[i]);
        result = arena_sprintf(cgen->arena, "%s._%d = %s", result, i, elem);
    }
    return arena_sprintf(cgen->arena, "%s }", result);
}

static const char *emit_idx_expr(CGen *cgen, const TTNode *node) {
    const char *object = cgen_emit_expr(cgen, node->idx_access.object);
    const char *idx = cgen_emit_expr(cgen, node->idx_access.idx);
    const Type *obj_type = node->idx_access.object->type;
    if (obj_type != NULL && obj_type->kind == TYPE_SLICE) {
        const char *elem_c_type = cgen_c_type_for(cgen, node->type);
        return arena_sprintf(cgen->arena, "((%s*)%s.data)[%s]", elem_c_type, object, idx);
    }
    return arena_sprintf(cgen->arena, "%s._data[%s]", object, idx);
}

static const char *emit_tuple_idx_expr(CGen *cgen, const TTNode *node) {
    const char *object = cgen_emit_expr(cgen, node->tuple_idx.object);
    return arena_sprintf(cgen->arena, "%s._%d", object, node->tuple_idx.elem_idx);
}

static const char *emit_type_conversion_expr(CGen *cgen, const TTNode *node) {
    return cgen_emit_expr(cgen, node->type_conversion.operand);
}

static const char *emit_module_access_expr(CGen *cgen, const TTNode *node) {
    const char *object = cgen_emit_expr(cgen, node->module_access.object);
    return arena_sprintf(cgen->arena, "%s._%s", object, node->module_access.member);
}

static const char *emit_struct_lit_expr(CGen *cgen, const TTNode *node) {
    const char *tname = cgen_c_type_for(cgen, node->type);
    const char *result = arena_sprintf(cgen->arena, "(%s){ ", tname);
    for (int32_t i = 0; i < BUF_LEN(node->struct_lit.field_names); i++) {
        ...
        const char *value = cgen_emit_expr(cgen, node->struct_lit.field_values[i]);
...
static const char *emit_struct_field_access_expr(CGen *cgen, const TTNode *node) {
    const char *object = cgen_emit_expr(cgen, node->struct_field_access.object);
...
static const char *emit_method_call_expr(CGen *cgen, const TTNode *node) {
    const char *recv = cgen_emit_expr(cgen, node->method_call.recv);
...
static const char *emit_heap_alloc_expr(CGen *cgen, const TTNode *node) {
    const char *pointee_type = cgen_c_type_for(cgen, node->type->ptr.pointee);
    const char *tmp = cgen_next_temp(cgen);
    const char *inner = cgen_emit_expr(cgen, node->heap_alloc.operand);
    cgen_emit_line(cgen, "%s *%s = rsg_heap_alloc(sizeof(%s));", pointee_type, tmp, pointee_type);
    cgen_emit_line(cgen, "*%s = %s;", tmp, inner);
    return tmp;
}

static const char *emit_address_of_expr(CGen *cgen, const TTNode *node) {
    const char *operand = cgen_emit_expr(cgen, node->address_of.operand);
    return arena_sprintf(cgen->arena, "&(%s)", operand);
}

static const char *emit_deref_expr(CGen *cgen, const TTNode *node) {
    const char *operand = cgen_emit_expr(cgen, node->deref.operand);
    return arena_sprintf(cgen->arena, "(*%s)", operand);
}

/** Emit a match expr as a temp var with an if-else chain. */
/** Emit a single guarded match arm (used inside do { } while(0) block). */
static void emit_match_arm_guarded(CGen *cgen, const TTNode *cond, const TTNode *guard,
                                   const TTNode *body, const TTNode *bindings, const char *result) {
    if (cond != NULL) {
        const char *cond_str = cgen_emit_expr(cgen, cond);
    ...
    if (bindings != NULL && bindings->kind == TT_BLOCK) {
        cgen_emit_block_stmts(cgen, bindings);
    }

    if (guard != NULL) {
        const char *guard_str = cgen_emit_expr(cgen, guard);
        const char *gwrapped = guard_str;
        if (guard_str[0] != '(') {
            gwrapped = arena_sprintf(cgen->arena, "(%s)", guard_str);
        }
        cgen_emit_line(cgen, "if %s {", gwrapped);
        cgen->indent++;
    }

    if (result != NULL) {
        const char *value = cgen_emit_expr(cgen, body);
        cgen_emit_line(cgen, "%s = %s;", result, value);
    } else {
...
static void emit_match_arm_simple(CGen *cgen, int32_t arm_idx, const TTNode *cond,
                                  const TTNode *body, const TTNode *bindings, const char *result) {
    if (cond != NULL) {
        const char *cond_str = cgen_emit_expr(cgen, cond);
        const char *wrapped = cond_str;
...
static const char *emit_match_expr(CGen *cgen, const TTNode *node) {
    const Type *type = node->type;

    cgen_emit_stmt(cgen, node->match_expr.operand);

    const char *result = NULL;
    if (type != NULL && type->kind != TYPE_UNIT) {
        result = cgen_next_temp(cgen);
        cgen_emit_line(cgen, "%s %s;", cgen_c_type_for(cgen, type), result);
    }
...
const char *cgen_emit_expr(CGen *cgen, const TTNode *node) {
    ...
    case TT_CHAR_LIT:
        return cgen_c_char_escape(cgen, node->char_lit.value);
    case TT_STR_LIT:
