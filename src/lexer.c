...
struct Lexer {
    ...
    int32_t pos;              // current byte offset
...
static char peek(const Lexer *l) {
    return (char)(l->pos < l->length ? l->source[l->pos] : '\0');
}

static char peek_next(const Lexer *l) {
    return (char)(l->pos + 1 < l->length ? l->source[l->pos + 1] : '\0');
}
...
static SrcLoc loc(const Lexer *l) {
    return (SrcLoc){.file = l->file, .line = l->line, .col = l->col};
}

static Token make_token(const Lexer *l, TokenKind kind, const char *start, int32_t len, SrcLoc s) {
    return (Token){
        .kind = kind,
        .lexeme = arena_strndup(l->arena, start, len),
        .length = len,
        .loc = s,
    };
}
...
static Token scan_number(Lexer *l, SrcLoc s) {
    ...
    int32_t len = (int32_t)(l->source + l->pos - start);
    Token t = make_token(l, is_float ? TOK_FLOAT_LIT : TOK_INT_LIT, start, len, s);

    // Strip underscores and parse value
    char buf[64];
    int32_t bi = 0;
    for (int32_t i = 0; i < len && bi < 63; i++) {
        if (start[i] != '_') {
            buf[bi++] = start[i];
        }
    }
    ...
    if (is_float) {
        t.lit.float_val = strtod(buf, NULL);
    } else {
        t.lit.int_val = strtoll(buf, NULL, 10);
    }
...
static Token scan_string(Lexer *l, SrcLoc s) {
    ...
    if (!has_interp) {
        ...
        if (peek(l) == '\0') {
            ...
            return make_token(l, TOK_ERROR, l->source + content_start - 1, l->pos - content_start + 1, s);
        }
    ...
    while (peek(l) != '"' && peek(l) != '\0') {
        ...
        int32_t seg_start = l->pos;
        SrcLoc seg_loc = loc(l);
        ...
        // Emit text segment
        char *text = extract_str_content(l, seg_start, l->pos);
        ...
        if (peek(l) == '{') {
            ...
            // Emit INTERP_START
            BUF_PUSH(l->pending, make_token(l, TOK_INTERP_START, "{", 1, loc(l)));
            ...
            // Emit INTERP_END
            BUF_PUSH(l->pending, make_token(l, TOK_INTERP_END, "}", 1, loc(l)));
        }
    }
    ...
    if (BUF_LEN(l->pending) > 0 && l->pending[BUF_LEN(l->pending) - 1].kind == TOK_INTERP_END) {
        BUF_PUSH(l->pending, make_str_token(l, "", loc(l)));
    }
...
Token lexer_next(Lexer *l) {
    ...
    if (l->pending != NULL && l->pending_pos < BUF_LEN(l->pending)) {
        return l->pending[l->pending_pos++];
    }
