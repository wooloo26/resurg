...
static void emit_preamble(CGen *cgen) {
    _emit(cgen, "// Generated by resurg pipeline - do not edit.\n");
    _emit(cgen, "#include <stdint.h>\n");
    _emit(cgen, "#include <stdbool.h>\n");
    _emit(cgen, "#include \"rsg_runtime.h\"\n\n");
}
...
static bool emit_top_level_wrapper(CGen *cgen, const HirNode *file) {
    ...
    _emit(cgen, "static void _rsg_top_level(void) {\n");
    cgen->indent++;
    for (int32_t i = 0; i < BUF_LEN(file->file.decls); i++) {
        if (is_top_level_stmt(file->file.decls[i])) {
